...
play a braille spinner + ``🧠 Thinking…(Ns)`` timer in a cool/calm color,
visually distinct from the warm vermilion waiting mascot ``=^_^=`` (F17). The raw
reasoning text is never shown. When thinking ends, a single persistent
breadcrumb ``💭 Thinking Ns`` is left in scrollback (F104).
...
Usage (TTY)::
    ...
    anim.stop()
    console.print(anim.render_breadcrumb())  # leaves 💭 Thinking Ns
...
# Braille spinner frames — cool-tone thinking-state exclusive, symbolically distinct from mascot =^_^= (F103)
BRAILLE_FRAMES: tuple[str, ...] = ("⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧")

# Cool color (steel blue / slate-blue, xterm SteelBlue3 range) — visually distinct from
# mascot vermilion #C84B31 (F103: thinking-state cool vs. waiting-state vermilion warm).
...
class ThinkingAnimation:
    ...
    def render_line(self) -> Text:
        ...
        Format: ``<braille> 🧠 Thinking… (<N>s)`` styled :data:`THINK_STYLE`;
        ...
        seconds = int(elapsed)
        return Text(f"{frame} 🧠 Thinking… ({seconds}s)", style=THINK_STYLE)

    def render_breadcrumb(self) -> Text:
        """Return the persistent end-of-thinking breadcrumb (F104).

        Format: ``💭 Thinking <N>s`` styled ``dim`` — a single-line trace left in
        ...
        return Text(f"💭 Thinking {seconds}s", style="dim")
